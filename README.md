...
```powershell
git clone https://github.com/<用户名>/tencent_docs_pom.git
cd tencent_docs_pom
...
1. 项目根目录放置 `upx.exe`（已包含在仓库中）
2. 执行 `python build\prepare_vendor.py`（自动复制 `vendor/app.py`）

### 一键打包

```powershell
# 标准版（含 Chromium，离线可用，约 500MB zip）
powershell -ExecutionPolicy Bypass -File build\nuitka_build.ps1

# Lite 版（首次运行自动下载 Chromium，约 85MB zip）
...
| `releases/WorkOrderAutomation-Nuitka-Windows.zip` | 发布 zip |
...
└── build/
    ├── nuitka_build.ps1   # Nuitka 一键打包（主推）
    ├── apply_upx.ps1      # UPX 二次压缩
    ├── prepare_vendor.py  # 复制 vendor/app.py
    └── stage_browsers.py  # Chromium 复制管理
```
...
**Q: 打包后汇总报错？**  
A: 确认 `vendor/app.py` 存在，运行 `python build/prepare_vendor.py` 复制。
